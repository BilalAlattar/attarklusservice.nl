...
// ─── validation (returns undefined for valid fields) ─────
...
export function ContactSection() {
  ...
  // ── core update: validates field immediately if already touched ──
  ...
  // ── name: block digits and symbols at input level ───────
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const clean = e.target.value.replace(/[\d!@#$%^&*()+={}\[\]|\\<>,.?/:;"~`_]/g, "");
    updateField("name", clean);
  };

  // ── phone: block letters and symbols at input level ─────
  ...
  // ── blur: mark touched and validate immediately ──────────
  ...
  const fieldCls = (field: keyof Form) => {
    ...
    return [
      "w-full border rounded-2xl px-4 py-3 text-sm outline-none transition-all duration-200",
      "focus:ring-2",
      hasError
    ...
    <section className="py-16 px-6 lg:px-10 bg-slate-50">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.form
          ...
          onSubmit={onSubmit}
          noValidate
          className="max-w-lg mx-auto bg-white rounded-[32px] shadow-elegant border border-slate-100 p-6 sm:p-8 space-y-4"
        >
          {/* Name */}
          <div>
            <input
